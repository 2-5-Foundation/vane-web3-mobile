...
export default function Home() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      try {
        const hasClearedAllStorage = localStorage.getItem(
          "vane-all-storage-cleared",
        );
        if (!hasClearedAllStorage) {
          ...
          console.log(
            "One-time cleanup: Cleared all app-related localStorage items",
          );
        }
      } catch (e) {
  ...
  useEffect(() => {
    ...
    const setupP2pWatch = () => {
      ...
      const isPrimaryWalletSender = (txStateMachine: {
        ...
        const normalizedPrimaryAddress =
          normalizeChainAddress(primaryWalletAddress);
        const normalizedSenderAddress = normalizeChainAddress(
          txStateMachine.senderAddress,
        );
      ...
      const isPrimaryWalletInvolved = (txStateMachine: {
        senderAddress: string;
        ...
        const normalizedReceiverAddress = normalizeChainAddress(
          txStateMachine.receiverAddress,
        );

        return (
          normalizedPrimaryAddress === normalizedSenderAddress ||
          normalizedPrimaryAddress === normalizedReceiverAddress
        );
      };
      ...
      const handleBackendEvent = (event: BackendEvent) => {
        ...
        if ("SenderRequestHandled" in event) {
          const txStateMachine = decodeTxStateMachine(
            event.SenderRequestHandled.data,
          );
        ...
        if ("ReceiverResponseHandled" in event) {
          const txStateMachine = decodeTxStateMachine(
            event.ReceiverResponseHandled.data,
          );
          if (isPrimaryWalletInvolved(txStateMachine)) {
            toast.success(
              `Receiver responded for sender ${txStateMachine.senderAddress}`,
            );
          }
          return;
        }
        ...
        if ("PeerDisconnected" in event) {
          ...
          if (primaryWalletAddress) {
            const normalizedPrimaryAddress =
              normalizeChainAddress(primaryWalletAddress);
            const normalizedDisconnectedAddress = normalizeChainAddress(
              event.PeerDisconnected.account_id,
            );
            if (normalizedPrimaryAddress === normalizedDisconnectedAddress) {
              toast.error(
                `Peer disconnected: ${event.PeerDisconnected.account_id}`,
                {
        ...
        if ("DataExpired" in event) {
          ...
          if (isPrimaryWalletInvolved(txStateMachine)) {
            toast.error(
              `Transaction expired for receiver ${txStateMachine.receiverAddress}`,
              {
                style: { background: "#fee2e2", color: "#991b1b" },
              },
            );
        ...
        if ("TxSubmitted" in event) {
          ...
          if (isPrimaryWalletInvolved(txStateMachine)) {
            toast.success(
              `Transaction submitted for receiver ${txStateMachine.receiverAddress}`,
            );
          }
          return;
        }
