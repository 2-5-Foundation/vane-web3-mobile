...
const USE_ANVIL =
  (typeof process !== "undefined" && process.env?.VITE_USE_ANVIL === "true") ||
  (typeof import.meta !== "undefined" &&
    (import.meta as any).env?.VITE_USE_ANVIL === "true");
...
export function reconstructSignedTransaction(
  ...
  async submitTx(tx: TxStateMachine): Promise<Uint8Array> {
    ...
    try {
      ...
      if (!tx.signedCallPayload)
        throw new Error(
          "No signed call payload found - transaction must be signed first",
        );
      if (!tx.callPayload)
        throw new Error(
          "No call payload found - transaction must be created first",
        );
      ...
      if (family === "ethereum") {
        ...
        if (USE_ANVIL) {
          const chainConfig =
            CHAIN_CONFIGS[
              tx.senderAddressNetwork as keyof typeof CHAIN_CONFIGS
            ];
          const publicClient = createPublicClient({
            chain: chainConfig.chain,
            transport: http(chainConfig.rpcUrl),
          });
          const serializedTxBytes =
            "ethereum" in tx.callPayload!
              ? tx.callPayload.ethereum.callPayload[1]
              : null;
          if (!serializedTxBytes)
            throw new Error("Invalid call payload for Ethereum transaction");
        ...
        if (!hashHex || typeof hashHex !== "string")
          throw new Error("Invalid hash from API");
      ...
      if (family === "bsc") {
        ...
        if (USE_ANVIL) {
          ...
          const serializedTxBytes =
            "bnb" in tx.callPayload! ? tx.callPayload.bnb.callPayload[1] : null;
          if (!serializedTxBytes)
            throw new Error("Invalid call payload for BSC transaction");
          const signatureBytes = tx.signedCallPayload;
          const signedTransactionHex = reconstructSignedTransaction(
            new Uint8Array(serializedTxBytes),
            new Uint8Array(signatureBytes),
      ...
      if (family === "polkadot") {
        throw new Error(
          "Polkadot submission not implemented in host functions yet",
        );
      }
      ...
      if (family === "solana") {
        if (USE_ANVIL) {
          ...
          if (!("solana" in tx.callPayload!))
            throw new Error("Invalid call payload for Solana transaction");
          const vmsg = VersionedMessage.deserialize(
            new Uint8Array(tx.callPayload.solana.callPayload),
          );
        ...
        if (!resp.ok)
          throw new Error(`API submitTx failed: ${resp.status}: ${resp}`);
        const data = await resp.json();
        const hashHex: string = data?.hash;
        if (!hashHex || typeof hashHex !== "string")
          throw new Error("Invalid hash from API");
      ...
      console.error(
        `Failed to submit transaction for ${tx.senderAddressNetwork}:`,
        error,
      );
      throw error;
    }
  ...
  async createTx(tx: TxStateMachine): Promise<TxStateMachine> {
    try {
      ...
      if (
        ...
        if (!resp.ok)
          throw new Error(`API prepareCreateTx failed: ${resp.status}`);
        const data = await resp.json();
      ...
      console.error(
        `Failed to create transaction for ${tx.senderAddressNetwork}:`,
        error,
      );
      throw error;
    }
...
export async function createTestTxEVM(
  tx: TxStateMachine,
): Promise<TxStateMachine> {
  // construct an unsigned ethereum transaction
  const chainConfig =
    CHAIN_CONFIGS[tx.senderAddressNetwork as keyof typeof CHAIN_CONFIGS];
  const publicClient = createPublicClient({
    chain: chainConfig.chain,
    transport: http(chainConfig.rpcUrl),
  });
  ...
  if (isNativeToken) {
    ...
    if (!tokenInfo) {
      throw new Error(
        `Invalid ERC20 token address for ${JSON.stringify(tx.token)}`,
      );
    }
  ...
  if (!signingPayload.startsWith("0x02"))
    throw new Error("Expected 0x02 typed payload");
...
export async function createTestTxBSC(
  tx: TxStateMachine,
): Promise<TxStateMachine> {
  const chainConfig =
    CHAIN_CONFIGS[tx.senderAddressNetwork as keyof typeof CHAIN_CONFIGS];
  ...
  if (isNativeToken) {
    ...
    if (!tokenInfo) {
      throw new Error(
        `Invalid BEP20 token address for ${JSON.stringify(tx.token)}`,
      );
    }
    const { address: tokenAddress, decimal } = tokenInfo;
  ...
  const signingPayload = serializeTransaction(fields) as Hex;
  if (!signingPayload.startsWith("0x"))
    throw new Error("Expected 0x legacy payload");
...
export async function createTestTxSolana(
  tx: TxStateMachine,
): Promise<TxStateMachine> {
  ...
  if (isNativeSol) {
    ...
    if (!tokenInfo) {
      throw new Error(
        `Invalid SPL token address for ${JSON.stringify(tx.token)}`,
      );
    }
    const { address: tokenAddress, decimal } = tokenInfo;
...
export function fromWire(wireTx: any): TxStateMachine {
  return {
    ...wireTx,
    inboundReqId:
      typeof wireTx.inboundReqId === "number" ? wireTx.inboundReqId : null,
    outboundReqId:
      typeof wireTx.outboundReqId === "number" ? wireTx.outboundReqId : null,
